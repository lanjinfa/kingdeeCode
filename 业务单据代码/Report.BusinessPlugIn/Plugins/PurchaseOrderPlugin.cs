...
namespace BOA.DJJX.Report.BusinessPlugIn.Plugins
{
    [HotUpdate]
    [Description("采购订单")]
    public class PurchaseOrderPlugin : AbstractBillPlugIn
    {
        ...
        public override void EntryBarItemClick(BarItemClickEventArgs e)
        {
            ...
            {
                var row = this.View.Model.GetEntryCurrentRowIndex("FPOOrderEntry");
        ...
        private void OpenTreasureChestForm(int row)
        {
            ...
            var orgInfo = this.View.Model.GetValue("FPurchaseOrgId");//采购组织
            if (orgInfo == null)
            {
                this.View.ShowErrMessage("采购组织不能为空！");
                return;
            }
            var orgId = (orgInfo as DynamicObject)["Id"];
            var supplier = this.View.Model.GetValue("FSupplierId");//供应商
            var supplierId = supplier == null ? 0 : (supplier as DynamicObject)["Id"];
