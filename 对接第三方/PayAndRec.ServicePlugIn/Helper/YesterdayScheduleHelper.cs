...
namespace BOA.YZW.PayAndRec.ServicePlugIn.Helper
{
    ...
    /// 收款单定时任务帮助类(获取昨天的数据)
    /// </summary>
    public static class YesterdayScheduleHelper
    {
        /// <summary>
        /// 获取昨天的收款单数据
        /// </summary>
...
'','0','1')";
            ...
            #region 获取历史明细 即 昨天的数据再获取一次 保证数据不会丢失(因为cbs接口返回的数据不是实时的)
            //var startTime = "2023-02-01";
...
'{xml01.OuterXml.Replace("'", "").Replace("-", "")}')";
                ...
                else
                {
                    ...
                    foreach (var item in result01.EREXPTRSZ)
                    {
                        ...
                        model["FDate"] = item.BNKTIM; //yesterdayDate;//业务日期
